      ...
      <div className="relative flex h-full w-full flex-col max-w-md mx-auto border-x border-border-dark/30 shadow-2xl shadow-black">
        ...
        <div className="flex-1 flex flex-col px-6 pb-8 -mt-6 relative z-10">
          ...
          <div className="space-y-3 mb-6">
            <button onClick={() => navigate(ROUTES.CLIENT_HOME)} className="w-full bg-primary hover:bg-primary-dark text-background-dark font-bold text-lg py-3.5 rounded-xl shadow-lg shadow-primary/20 transition-all flex justify-center items-center gap-2">
              <span>Log In as Client</span>
              <span className="material-symbols-outlined text-[20px]">person</span>
            </button>
            <button onClick={() => navigate(ROUTES.DRIVER_PROFILE)} className="w-full bg-surface-dark hover:bg-surface-dark/80 border border-border-dark text-white font-bold text-lg py-3.5 rounded-xl transition-all flex justify-center items-center gap-2">
              <span>Log In as Driver</span>
              <span className="material-symbols-outlined text-[20px]">directions_car</span>
            </button>
            <button onClick={() => navigate(ROUTES.ADMIN_DASHBOARD)} className="w-full bg-surface-dark hover:bg-surface-dark/80 border border-border-dark text-white font-bold text-lg py-3.5 rounded-xl transition-all flex justify-center items-center gap-2">

...
export const BottomNavClient: React.FC<NavProps> = ({ active }) => (
  <div className="fixed bottom-0 left-0 right-0 bg-[#181611]/95 backdrop-blur-xl border-t border-white/5 px-6 py-4 z-50 max-w-md mx-auto">
    <div className="flex items-center justify-around">
      <Link to="/client/home" className={`flex flex-col items-center gap-1 group w-16 ${active === 'home' ? 'text-primary' : 'text-gray-400'}`}>
        <span className={`material-symbols-outlined text-[26px] ${active === 'home' ? 'filled' : ''}`}>home</span>
        <span className="text-[10px] font-medium">Home</span>
      </Link>
      <Link to="/client/trips" className={`flex flex-col items-center gap-1 group w-16 ${active === 'trips' ? 'text-primary' : 'text-gray-400'}`}>
        <span className={`material-symbols-outlined text-[26px] ${active === 'trips' ? 'filled' : ''}`}>history</span>
        <span className="text-[10px] font-medium">Trips</span>
      </Link>
      <Link to="/client/profile" className={`flex flex-col items-center gap-1 group w-16 ${active === 'profile' ? 'text-primary' : 'text-gray-400'}`}>
...
export const BottomNavAdmin: React.FC<NavProps> = ({ active }) => (
  <div className="fixed bottom-0 left-0 right-0 bg-surface-dark/95 backdrop-blur-xl border-t border-white/5 px-6 py-4 z-50 max-w-md mx-auto">
    <div className="flex justify-between items-center max-w-lg mx-auto">
      <Link to="/admin/dashboard" className={`flex flex-col items-center gap-1 ${active === 'home' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'}`}>
        <span className={`material-symbols-outlined ${active === 'home' ? 'filled' : ''}`}>dashboard</span>
        <span className="text-[10px] font-semibold">Home</span>
      </Link>
      <Link to="/admin/bookings" className={`flex flex-col items-center gap-1 ${active === 'bookings' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'}`}>
        <span className={`material-symbols-outlined ${active === 'bookings' ? 'filled' : ''}`}>calendar_today</span>
        <span className="text-[10px] font-medium">Bookings</span>
      </Link>
      <Link to="/admin/clients" className={`flex flex-col items-center gap-1 ${active === 'clients' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'}`}>
        <span className={`material-symbols-outlined ${active === 'clients' ? 'filled' : ''}`}>group</span>
        <span className="text-[10px] font-medium">Clients</span>
      </Link>
      <Link to="/admin/settings" className={`flex flex-col items-center gap-1 ${active === 'settings' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'}`}>
